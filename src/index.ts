...
async function bootstrap() {
  ...
  logger.info(`🌐 Web server running on 0.0.0.0:${env.PORT}`);

  if (env.WEBHOOK_DOMAIN) {
    const url = new URL('/webhook', env.WEBHOOK_DOMAIN).toString();

    await bot.api.setWebhook(url, {
      secret_token: env.WEBHOOK_SECRET,
    });

    logger.info(`🔗 Webhook set successfully to: ${url}`);
  } else {
    await bot.api.deleteWebhook();

    bot.start({
      onStart: (i) => logger.info(`🤖 Bot started polling as @${i.username}`),
    });
  }
