...
courseComposer.on('message:text',async(ctx,next):Promise<void>=>{
 ...
 if(ctx.session.step==='ADD_RESOURCE_SEMESTER'){
   ...
   await CourseService.createResource({...p,semester:text,submittedById:ctx.dbUser.id});ctx.session.step='IDLE';ctx.session.pendingResource=undefined;await ctx.reply('✅ لینک با موفقیت ثبت شد.');return;
 }
 await next();
});
